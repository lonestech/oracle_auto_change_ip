...
# 更新dns解析
def update_cloudflare_dns(record_name, ip_address, api_token, cloudflare_email, cloudflare_zone_name):
    # Cloudflare API 认证头部
    headers = {
    ...
    # 获取区域ID
    response = requests.get(cloudflare_api_url + 'zones', headers=headers)
    if response.status_code != 200:
        log_info(f"Failed to get zone ID for {cloudflare_zone_name}. Status code: {response.status_code}")
        return
    try:
        zones = response.json()['result']
        if not zones:
            log_info(f"No zones found for {cloudflare_zone_name}")
            return
        zid = next((zone['id'] for zone in zones if zone['name'] == cloudflare_zone_name), None)
        if zid is None:
            log_info(f"Zone {cloudflare_zone_name} not found")
            return
    except (KeyError, IndexError, TypeError):
        log_info(f"Error parsing zone ID response for {cloudflare_zone_name}")
        return

    # 获取记录ID
    response = requests.get(cloudflare_api_url + 'zones/' + zid + '/dns_records?name=' + record_name, headers=headers)
    if response.status_code != 200:
        log_info(f"Failed to get record ID for {record_name}. Status code: {response.status_code}")
        return
    try:
        records = response.json()['result']
        if not records:
            log_info(f"No records found for {record_name} in zone {cloudflare_zone_name}")
            return
        rid = records[0]['id']
    except (KeyError, IndexError, TypeError):
        log_info(f"Error parsing record ID response for {record_name}")
        return
    ...
    if response.status_code != 200:
        log_info(f"Failed to update DNS record for {record_name}. Status code: {response.status_code}")
    ...
    for account in accounts:
        ...
        for instance in instances:
            ...
            for vnic in vnics:
                ...
                # 更新DNS解析
                update_cloudflare_dns(domain,new_public_ip, account['cloudflare']['cloudflare_api_token'],account['cloudflare']['cloudflare_email'], account['cloudflare']['zonenames'])
